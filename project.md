...
┌─────────────────────────────────────────────────────────────────────────────┐
│                        LINGUABOARD v1.0                                     │
│                   ПРОМПТ-ДОКУМЕНТ (ОПИСАНИЕ ПРОЕКТА)                        │
│                                                                             │
│  Дата:         16 мая 2026                                                   │
│  Стек:         Next.js + TypeScript + Supabase + DeepSeek API + Vercel      │
│  Автор:        [Твоё имя], Стрелец 🔥                                        │
│  Цель:         AI Security Architect                                         │
└─────────────────────────────────────────────────────────────────────────────┘

═══════════════════════════════════════════════════════════════════════════════
1. ЧТО ТАКОЕ LINGUABOARD
═══════════════════════════════════════════════════════════════════════════════

  LinguaBoard — это персональная операционная система для дисциплины
  и изучения языков. Два крыла в одном проекте:

  📋 TASKFLOW — Канбан-доска задач (To-Do)
  🌐 LINGUALAB — Изучение языков через канбан слов + AI-генерацию

  AI-ассистент (DeepSeek) генерирует карточки слов, квизы и анализирует
  прогресс. Пользователь пишет ЛЮБУЮ тему — AI создаёт материалы.
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  Пользователь сам выбирает, какой язык учить, и сам ставит уровень:
  🟢 Beginner / 🟡 Intermediate / 🔴 Advanced

═══════════════════════════════════════════════════════════════════════════════
3. TASKFLOW — УПРАВЛЕНИЕ ЗАДАЧАМИ
═══════════════════════════════════════════════════════════════════════════════

  Доски задач (AITU, Спорт, Проекты, Личное).
  Каждая доска — 4 колонки:
  ...
  Задача имеет:
    ...
    • Дедлайн (дата + время)
    • Теги (#ai, #китайский, #диплом)
    • Чеклист подзадач
    • Оценка времени (минуты)
    • Связь с языком (кнопка «Сгенерировать слова»)

  Связь TaskFlow → LinguaLab:
    Задача «Выучить 10 слов по IT» → кнопка → AI генерирует карточки →
    они падают в колонку «Новые» нужной языковой доски.

═══════════════════════════════════════════════════════════════════════════════
4. LINGUALAB — ИЗУЧЕНИЕ ЯЗЫКОВ
═══════════════════════════════════════════════════════════════════════════════

  КАНБАН СЛОВ:
    Каждый язык — отдельная доска с 4 колонками:
      📥 Новые     — только что добавленные / сгенерированные
      📖 Изучаю    — в процессе изучения
      🔄 Повторяю  — знаю, но нужно освежать (Spaced Repetition)
      ✅ Выучено   — уверенное знание

    Слово имеет:
      • word              — слово на языке
      • translation       — перевод на русский
      • phonetic          — транскрипция (пиньинь для ZH, IPA для остальных)
      • example_sentence  — пример использования
      • part_of_speech    — часть речи
      • difficulty        — beginner / intermediate / advanced
      • mastery_level     — 0-5 (алгоритм SM-2)
      • next_review_at    — когда повторить
      • tags              — темы, по которым сгенерировано

  AI-ГЕНЕРАТОР СЛОВ:
    Пользователь вводит:
      • Язык (en/es/fr/zh)
      • Тему (ЛЮБУЮ — "путешествия", "IT", "кулинария", "медицина"…)
      • Количество слов (1-50)
      • Сложность (beginner/intermediate/advanced)

    AI (DeepSeek) возвращает готовые карточки.
    Они сохраняются в БД и падают в колонку «Новые».

  РЕЖИМ ФЛЕШ-КАРТОЧЕК:
    Полноэкранный режим пролистывания.
    Лицевая сторона: слово + транскрипция.
    Обратная сторона: перевод + пример.
    Кнопки: «Знаю» (→ Повторяю) / «Не знаю» (→ остаётся в Изучаю).

  КВИЗЫ (3 ТИПА):
    Типы:
      1. vocabulary  — проверка знания слов (multiple choice)
      2. grammar     — проверка грамматики
      3. mixed       — всё вперемешку

    Пользователь вводит:
      • Язык
      • Тему (любую)
      • Количество вопросов (по умолчанию 10)
      • Сложность
      • Тип квиза

    AI генерирует квиз.
    Результаты сохраняются: quiz_attempts + quiz_answers.
    Последние 5 квизов видны на дашборде.

  СПЕЙСД РЕПЕТИШН (SM-2):
    Алгоритм интервальных повторений.
    После каждого квиза/повторения:
      «Знаю»    → интервал растёт (1 → 3 → 7 → 14 → 30 дней)
      «Не знаю» → сброс до 1 дня

    На дашборде виджет: «Сегодня повторить: X слов».

  СЛОВАРЬ:
    Таблица всех слов с фильтрами:
      по колонке, сложности, дате, тегам.
    Поиск по слову или переводу.
  ...
  ЧЕЛЛЕНДЖИ:
    «30 дней китайского», «100 слов за неделю».
    Календарь с зелёными отметками выполненных дней.
    Счётчик: «Дней подряд: 14».
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  ДОРОЖНАЯ КАРТА НАВЫКОВ:
    ...
    Уровень 1: Prompt Engineering       ✅ ГОТОВО
      • Structured Output (JSON)
      • Генерация карточек слов
      • Генерация квизов

    Уровень 2: Function Calling          🔜 ТЕКУЩИЙ
      • 9 инструментов (Tools):
        1. generate_word_cards   — создать карточки
        2. create_quiz           — создать квиз
        3. get_due_reviews       — слова на повторение
        4. get_user_stats        — статистика
        5. search_similar_words  — семантический поиск
        6. get_grammar_notes     — поиск по заметкам
        7. add_grammar_note      — создать заметку
        8. create_challenge      — создать челлендж
        9. get_weak_words        — проблемные слова

    Уровень 3: RAG (Retrieval)           📅 МЕСЯЦ 3
      • Embeddings + pgvector
      • Семантический поиск по словам
      • Поиск по заметкам

    Уровень 4: Multi-Agent               📅 МЕСЯЦ 4
      • 4 агента: Учитель, Тестировщик, Аналитик, Оркестратор
      • Каждый со своей ролью и набором Tools

    Уровень 5: Agentic Security          🏆 МЕСЯЦ 5-6
      • Защита от Prompt Injection
      • Row-Level Security (RLS)
      • Аудит всех вызовов агента (agent_audit_log)
      • Penetration Testing своего агента

═══════════════════════════════════════════════════════════════════════════════
6. ДАШБОРД (ГЛАВНЫЙ ЭКРАН)
═══════════════════════════════════════════════════════════════════════════════

  ВИДЖЕТЫ:

    ┌─────────────────────────────────────────────────────────┐
    │  ПРИВЕТ, [Имя]!                    🔥 Стрик: 14 дней    │
    ├─────────────────────────────────────────────────────────┤
    │                                                         │
    │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │
    │  │ ЗАДАЧ НА     │  │ СЛОВ НА      │  │ АКТИВНЫХ     │   │
    │  │ СЕГОДНЯ      │  │ ПОВТОРЕНИЕ   │  │ ЧЕЛЛЕНДЖЕЙ   │   │
    │  │   5          │  │   12         │  │   2          │   │
    │  └──────────────┘  └──────────────┘  └──────────────┘   │
    │                                                         │
    │  ┌──────────────────────────────────────────────────┐   │
    │  │  📊 НЕДЕЛЬНАЯ АКТИВНОСТЬ                         │   │
    │  │  [столбчатая диаграмма: ПН-ВС]                   │   │
    │  └──────────────────────────────────────────────────┘   │
    │                                                         │
    │  ┌──────────────────────┐  ┌────────────────────────┐   │
    │  │  🇨🇳 КИТАЙСКИЙ        │  │  🧠 ПОСЛЕДНИЕ КВИЗЫ     │   │
    │  │  Слов: 250           │  │  IT-безопасность 8/10  │   │
    │  │  Выучено: 80 (32%)   │  │  Путешествия    7/10  │   │
    │  │  Точность: 78%       │  │  Грамматика     9/10  │   │
    │  └──────────────────────┘  └────────────────────────┘   │
    └─────────────────────────────────────────────────────────┘

═══════════════════════════════════════════════════════════════════════════════
7. LISTENING & SPEAKING — COMING SOON
═══════════════════════════════════════════════════════════════════════════════

  Эти функции НЕ входят в текущую версию.
  В интерфейсе отображаются как заглушка с бейджем «🚀 Coming Soon».

  🎧 Аудирование:
    "Слушайте произношение носителей и отвечайте на вопросы.
     AI-озвучка появится в следующем обновлении."

  🎤 Разговорная практика:
    "Тренируйте произношение с AI-ассистентом.
     Распознавание речи скоро появится."
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  ГРАФИКИ:            Recharts
  БЭКЕНД:             Next.js API Routes (единый с фронтом)
  БАЗА ДАННЫХ:        Supabase PostgreSQL + pgvector
  АВТОРИЗАЦИЯ:        Supabase Auth
  AI:                 DeepSeek API (генерация слов, квизов)
  ДЕПЛОЙ:             Vercel

═══════════════════════════════════════════════════════════════════════════════
9. ЦВЕТОВАЯ СХЕМА (ТЁМНАЯ ТЕМА)
═══════════════════════════════════════════════════════════════════════════════

  Фон:                #0F0F1A (тёмно-синий)
  Карточки:           #1A1A2E
  Акцент (Огонь):     #FF6B35 (оранжевый)
  Вторичный:          #4ECDC4 (бирюзовый)
  Текст:              #E4E4E7

  Приоритет 1:        #FF4444 (красный)
  Приоритет 2:        #FFAA00 (жёлтый)
  Приоритет 3:        #44AAFF (синий)

  Колонка «Новые»:    #2D2D44
  Колонка «Изучаю»:   #FF6B35 (оранжевый)
  Колонка «Повторяю»: #4ECDC4 (бирюзовый)
  Колонка «Выучено»:  #44BB44 (зелёный)
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  ☐ Профиль с авто-созданием при регистрации
  ☐ Создание досок задач (TaskFlow)
  ☐ Drag & Drop задач между колонками
  ☐ Создание языковых досок (LinguaLab)
  ☐ Drag & Drop слов между колонками
  ☐ AI-генерация карточек слов (любая тема)
  ☐ AI-генерация квизов (10 вопросов по умолчанию)
  ☐ Прохождение квиза с сохранением результата
  ☐ Spaced Repetition (next_review_at)
  ☐ Дашборд со статистикой
  ☐ Деплой на Vercel

═══════════════════════════════════════════════════════════════════════════════
                        КОНЕЦ ПРОМПТ-ДОКУМЕНТА
═══════════════════════════════════════════════════════════════════════════════